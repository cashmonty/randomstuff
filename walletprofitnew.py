    ...
    with open('profitability_results.csv', 'w', newline='') as csvfile:
        ...
        for wallet_address in wallet_addresses:
            ...
            for tx in result['result']:
                # Filter transactions involving Uniswap and WETH
                if tx['to_address'] == uniswap_router_address or tx['from_address'] == uniswap_router_address:
                    if tx['token_address'] == weth_address:
                        total_trades += 1  # Increment total trades

                        # Calculate profit based on the transaction value
                        value_in_ether = float(tx['value']) / 1e18  # Convert value from Wei to Ether
                        profit = value_in_ether  # Profit in Ether
